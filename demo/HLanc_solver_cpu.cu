...
struct dcsrmv_wrapper_t {
	dcsrmv_wrapper_t ( op_t const & op ) : op ( op ), seq ( 0 ) { }
...
int main ( int argc, char * argv[] ) {
	if ( 5 != argc ) {
		HLanc::warn ( "Usage: %s <bin|mm|pp> <matrix_file> <nev> <ncv>\n", argv[0] );
		return 1;
	}
	...
	HLanc::dcsrmv_operator_mkl op ( dcsr_matrix );
	...
	int ret = HLanc::ds_solver_mkl ( n, nev, ncv, values, vectors, n, dcsrmv_wrapper_t < HLanc::dcsrmv_operator_mkl > ( op ) );
